...
existing_capacities = pd.read_csv("inputs/existing_capas.csv", index_col=0, squeeze=True, header=None)
maximum_capacities = pd.read_csv("inputs/max_capas.csv", index_col=0, squeeze=True, header=None)
capex = pd.read_csv("inputs/annuities.csv", index_col=0, squeeze=True, header=None)
capex_storage = pd.read_csv("inputs/str_annuities.csv", index_col=0, squeeze=True, header=None)
demand = pd.read_csv("inputs/demand2006.csv", index_col=0, squeeze=True, header=None)
lake_inflows = pd.read_csv("inputs/lake_inflows.csv", index_col=0, squeeze=True, header=None)
gene_river = pd.read_csv("inputs/run_of_river.csv", index_col=0, squeeze=True, header=None)
reserve_requirements = pd.read_csv("inputs/reserve_requirements.csv", index_col=0, squeeze=True, header=None)
...
model.months = Set(initialize=["jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec"])
model.generations = Set(initialize=["pv", "onshore", "offshore", "river", "lake", "biogas", "gas"])
...
model.storage_energy_capacity = Var(model.storages, within=NonNegativeReals, initialize=0)  # STORED
...
def generation_vre_constraint_rule(model, t, vre):
    return model.generation[vre, t] == model.capacities[vre] * production_profile[vre][t] # RENAME apacity_factor
...
def storing_constraint_rule(model, t):
    return model.stored[model.storages, t + 1] == model.stored[model.storages, t]\
           + model.storage[model.storages, t] * eta_in[model.storages]\
           - model.generation[model.storages, t] / eta_out[model.storages]
...
def storage_constraint_rule(model):
    return model.stored[model.storages, first_hour] == model.stored[model.storages, last_hour] \
           + model.storage[model.storages, last_hour] * eta_in[model.storages] \
           - model.generation[model.storages, last_hour] / eta_out[model.storages]

## lake_res(m)..                    lake_inflows(m)         =g=     sum(h$(month(h) = ord(m)),GENE('lake',h))/1000;
def lake_reserve_constraint_rule(model, months):
    return lake_inflows[months] >= sum(model.generation['lake', t] for t in months_hours[months]) / 1000
...
def objective_rule(model):
    return sum((model.capacities[tec] - existing_capacities[tec]) * capex[tec] for tec in model.technologies) \
           ...
           + sum(sum(model.generation[tec, hour] * vOM[tec] for hour in model.time) for tec in model.technologies)

model.generation_vre_constraint = Constraint(model.time, model.vre, rule=generation_vre_constraint_rule)
model.generation_capacity_constraint = Constraint(model.time, model.vre, rule=generation_capacity_constraint_rule)
model.combustion = Constraint(model.time, rule=combustion_constraint_rule)
model.frr_capacity_constraint = Constraint(model.time, model.frr, rule=frr_capacity_constraint_rule)
## TODO: CONSTRAINT NOT WORKING
# model.storing_constraint = Constraint(model.time, rule=storing_constraint_rule)
## TODO: CONSTRAINT NOT WORKING
# model.storage_constraint = Constraint(rule=storage_constraint_rule)
## TODO: CONSTRAINT NOT WORKING
# model.lake_reserve_constraint = Constraint(model.months, rule=lake_reserve_constraint_rule)
model.stored_capacity_constraint = Constraint(model.time, model.storages, rule=stored_capacity_constraint)
model.storage_capacity_1_constraint = Constraint(model.time, model.storages, rule=storage_capacity_1_constraint_rule)
model.storage_capacity_2_constraint = Constraint(model.storages, rule=storage_capacity_2_constraint_rule)
model.biogas_constraint = Constraint(model.time, rule=biogas_constraint_rule)
model.reserves_constraint = Constraint(model.time, rule=reserves_constraint_rule)
model.adequacy_constraint = Constraint(model.time, rule=adequacy_constraint_rule)
